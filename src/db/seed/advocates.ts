...
// Fixed random slice function
const randomSpecialty = (): [number, number] => {
  const n = specialties.length;
  if (n < 2) return [0, n];
  const start = Math.floor(Math.random() * (n - 1));
  const end = start + 1 + Math.floor(Math.random() * (n - start));
  return [start, Math.min(end, n)]; // ensure end <= n
};

// Optional roles for advocates
const roles = ["Nurse", "Psychologist", "Social Worker", "Therapist", "Psychiatrist"];
